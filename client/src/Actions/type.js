//Press 페이지 -----------------------------------
export const PRESS_LOAD_MAIN = "PRESS_LOAD_MAIN";
export const PRESS_FAIL_MAIN = "PRESS_FAIL_MAIN";
export const PRESS_LOAD_PRESS = "PRESS_LOAD_PRESS";
//Media 페이지 -----------------------------------
export const MEDIA_LOAD_MAIN = "MEDIA_LOAD_MAIN";
export const MEDIA_FAIL_MAIN = "MEDIA_FAIL_MAIN";
export const MEDIA_LOAD_MEDIA = "MEDIA_LOAD_MEDIA";
//Mail 페이지 -----------------------------------
export const MAIL_FAIL = "MAIL_FAIL";
export const MAIL_SUCCESS = "MAIL_SUCCESS";
//Board 페이지 -----------------------------------
export const BOARD_LOAD = "MAIL_FAIL";
export const BOARD_FAIL = "MAIL_SUCCESS";
export const BOARD_DETAIL_LOAD = "BOARD_DETAIL_LOAD";
export const BOARD_WRITE_SUCCESS = "BOARD_WRITE_SUCCESS";
export const BOARD_UPDATE_SUCCESS = "BOARD_UPDATE_SUCCESS";
export const BOARD_DELETE_SUCCESS = "BOARD_DELETE_SUCCESS";
export const BOARD_FILE_UPLOAD = "BOARD_FILE_UPLOAD";
export const BOARD_DOWNLOAD_FILE = "BOARD_DOWNLOAD_FILE";
export const BOARD_FILE_DELETE = "BOARD_FILE_DELETE";
export const BOARD_HIT_REGIST = "BOARD_HIT_REGIST";
